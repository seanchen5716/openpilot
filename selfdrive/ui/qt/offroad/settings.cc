...
DevicePanel::DevicePanel(QWidget* parent) : QWidget(parent) {
  ...
  auto dcamBtn = new ButtonControl("駕駛鏡頭", "預覽",
  ...
  auto resetCalibBtn = new ButtonControl("重新校正", "校正", resetCalibDesc);
  connect(resetCalibBtn, &ButtonControl::clicked, [=]() {
    if (ConfirmationDialog::confirm("確定要重新校正？將會重新啟動機器", this)) {
  ...
  if (!params.getBool("Passive")) {
    retrainingBtn = new ButtonControl("重看訓練指南", "查看", "오픈파일럿에 대한 규칙, 기능, 제한내용 등을 확인하세요.");
    connect(retrainingBtn, &ButtonControl::clicked, [=]() {
      if (ConfirmationDialog::confirm("您想查看訓練指南嗎？", this)) {
  ...
  QPushButton *reboot_btn = new QPushButton("重新啟動");
  reboot_btn->setObjectName("reboot_btn");
  power_layout->addWidget(reboot_btn);
  QObject::connect(reboot_btn, &QPushButton::clicked, [=]() {
    if (ConfirmationDialog::confirm("確定要重新啟動嗎？", this)) {
      Hardware::reboot();
    }
  });
...
SoftwarePanel::SoftwarePanel(QWidget* parent) : QWidget(parent) {
  ...
  lastUpdateLbl = new LabelControl("檢查是否有新版本", "", "");
  updateBtn = new ButtonControl("檢查並更新", "");
  connect(updateBtn, &ButtonControl::clicked, [=]() {
    ...
    if (!last_ping.length()) {
      desc += QString("沒有網路，請連接網路並執行檢查");
    } else if (commit_local == commit_remote) {
      desc += QString("本機與遠端版本相同，無需更新");
    } else {
      desc += QString("有更新，請按下確定按鈕執行更新");
    }
  ...
  auto uninstallBtn = new ButtonControl(getBrand() + " 刪除", "刪除");
  connect(uninstallBtn, &ButtonControl::clicked, [=]() {
    if (ConfirmationDialog::confirm("您確定要刪除嗎？", this)) {
  ...
  QPushButton *presetoneload_btn = new QPushButton("載入預設1");
  ...
  QObject::connect(presetoneload_btn, &QPushButton::clicked, [=]() {
    if (ConfirmationDialog::confirm("是否載入預設1", this)) {
      QProcess::execute("/data/openpilot/selfdrive/assets/addon/script/load_preset1.sh");
    }
  });

  QPushButton *presetonesave_btn = new QPushButton("預設1存檔");
  ...
  QObject::connect(presetonesave_btn, &QPushButton::clicked, [=]() {
    if (ConfirmationDialog::confirm("是否要存檔至預設1？", this)) {
  ...
  QPushButton *presettwoload_btn = new QPushButton("載入預設2");
  ...
  QObject::connect(presettwoload_btn, &QPushButton::clicked, [=]() {
    if (ConfirmationDialog::confirm("是否載入預設2", this)) {
      QProcess::execute("/data/openpilot/selfdrive/assets/addon/script/load_preset2.sh");
    }
  });

  QPushButton *presettwosave_btn = new QPushButton("預設2存檔");
  ...
  QObject::connect(presettwosave_btn, &QPushButton::clicked, [=]() {
    if (ConfirmationDialog::confirm("是否存檔至預設2？", this)) {
      QProcess::execute("/data/openpilot/selfdrive/assets/addon/script/save_preset2.sh");
    }
  });

  auto paraminit_btn = new ButtonControl("參數初始化", "執行");
  ...
  auto gitresetbtn = new ButtonControl("Git Reset", "執行");
  QObject::connect(gitresetbtn, &ButtonControl::clicked, [=]() {
    if (ConfirmationDialog::confirm("強制將本機修改的值初始化，並更新至遠端最新版本", this)){
  ...
  auto gitpullcanceltbtn = new ButtonControl("取消Git Pull", "執行");
  QObject::connect(gitpullcanceltbtn, &ButtonControl::clicked, [=]() {
    if (ConfirmationDialog::confirm("恢復至Git Pull之前的狀態，確定要執行嗎？", this)){
  ...
  auto pandaflashingtbtn = new ButtonControl("更新熊貓", "執行");
  QObject::connect(pandaflashingtbtn, &ButtonControl::clicked, [=]() {
    if (ConfirmationDialog::confirm("更新熊貓的期間，將快速閃爍綠燈，切勿斷開電源或強制關機，要繼續執行嗎？", this)) {
...
QWidget * network_panel(QWidget * parent) {
  ...
  auto wifiBtn = new ButtonControl("WiFi設定", "打開");
  QObject::connect(wifiBtn, &ButtonControl::clicked, [=]() { HardwareEon::launch_wifi(); });
  layout->addWidget(wifiBtn);
  layout->addWidget(horizontal_line());

  auto tetheringBtn = new ButtonControl("網路分享", "打開");
...
UserPanel::UserPanel(QWidget* parent) : QWidget(parent) {
  ...
  auto calokbtn = new ButtonControl("強制校正", "執行");
  ...
  layout->addWidget(horizontal_line());
  layout->addWidget(new LabelControl("PANDA值", ""));
  ...
  auto peditbtn = new ButtonControl("PANDA值修改", "執行");
...
TuningPanel::TuningPanel(QWidget* parent) : QWidget(parent) {
  ...
  layout->addWidget(new LabelControl("縱向駕駛控制", ""));
...
SettingsWindow::SettingsWindow(QWidget *parent) : QFrame(parent) {
  ...
  QList<QPair<QString, QWidget *>> panels = {
    ...
    {"版本", new SoftwarePanel(this)},
    {"使用者設定", new UserPanel(this)},
    {"調整", new TuningPanel(this)},
  };
